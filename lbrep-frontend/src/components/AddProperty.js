...
function AddProperty() {
    ...
    function ReducerFuction(draft, action) {
        switch (action.type) {
            ...
            case "changeSendRequest":
                draft.sendRequest = true; // ✅ set to boolean true
                break;

            case "resetSendRequest":
                draft.sendRequest = false; // ✅ reset after use
                break;
    ...
    useEffect(() => {
        console.log("useEffect fired with sendRequest:", state.sendRequest); // <-- add this
            ...
            async function AddProperty() {
                ...
                try {
                    ...
                    if (e.response) {
                        ...
                        console.log("No response received from server (network error?)");
                    }
                }
            }
            AddProperty();
    ...
    //             dispatch({ type: "resetSendRequest" });
    //             navigate("/listings");
    //         }, 2000);
        ...
        <form onSubmit={FormSubmit}>
            ...
            <Grid item container justifyContent={'space-between'}>
                ...
                <Grid item xs={5} style={{ marginTop: "1rem" }} >
                    <TextField
                        ...
                        onBlur={(e) =>
                            dispatch({
                                type: "catchRentalFrequencyErrors",
                                rentalFrequencyChosen: e.target.value,
                            })
                        }
                        error={state.rentalFrequencyErrors.hasErrors ? true : false}
                        helperText={state.rentalFrequencyErrors.errorMessage}
                        select
                        SelectProps={{
                            native: true,
                        }}
            ...
            {state.listingTypeValue === "Office" ? (
                ...
                <Grid item container style={{ marginTop: "1rem" }} >
                    <TextField
                        ...
                        error={state.roomsErrors.hasErrors ? true : false}
            ...
            <Grid item container justifyContent={'space-between'}>
                <Grid item xs={2} style={{ marginTop: "1rem" }} >
                    <FormControlLabel
                        control={<Checkbox checked={state.furnishedValue} onChange={(e) =>
                            dispatch({
                ...
                <Grid item xs={2} style={{ marginTop: "1rem" }} >
                    <FormControlLabel
                        control={<Checkbox checked={state.cctvValue} onChange={(e) =>
                            dispatch({
