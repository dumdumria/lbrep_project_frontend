...
function Login() {
		...
		if (state.sendRequest) {
			...
			async function SignIn() {
				try {
					...
					// Update global state with token immediately
					GlobalDispatch({
						type: "catchToken",
						tokenValue: response.data.auth_token,
					});
	...
	}, [state.sendRequest, GlobalDispatch]);
		...
		if (state.token !== "") {
			...
			async function GetUserInfo() {
				try {
					...
					// Update global state with all user information AND token
					GlobalDispatch({
						type: "userSignsIn",
						usernameInfo: response.data.username,
						emailInfo: response.data.email,
						IdInfo: response.data.id,
						tokenValue: state.token, // Make sure to pass the token
					});
